...
void FOnlineAchievementsPico::GetWriteAchievementCountValue(FVariantData VariantData, int64& OutData) const
{
	...
	case EOnlineKeyValuePairDataType::Int32:
	{
		...
		OutData = static_cast<int64>(Value);
		break;
	}
		...
		PicoSubsystem.AddRequestDelegate(RequestID, FPicoMessageOnCompleteDelegate::CreateLambda([this, PicoPlayerId, Delegate](FOnlineMessageHandle Message, bool bIsError)
			{
				...
				TArray<FOnlineAchievement>* ExistAchievements = PlayerAchievements.Find(PicoPlayerId);
				for (int32 i = 0; i < AchievementsForPlayer.Num(); i++)
				{
					ExistAchievements->Add(AchievementsForPlayer[i]);
				}
				if (OnlinePlatformInterfacePico::AchievementProgressArray_HasNextPage(Message.JavaMessage))
				{
					UE_LOG(LogTemp, Log, TEXT("AchievementProgressArray_GetNextUrl "));
				...
				{
					TArray<FOnlineAchievement>* NowExistAchievements = PlayerAchievements.Find(PicoPlayerId);
					TSet<FString> NowExistAchievementsName;
					for (auto Achievement : *NowExistAchievements)
					{
						NowExistAchievementsName.Add(Achievement.Id);
					}

					for (auto const& it : AchievementDescriptions)
					{
						auto bFoundAchievement = NowExistAchievementsName.Find(it.Key);
						if (bFoundAchievement == nullptr)
						{
							UE_LOG(LogTemp, Log, TEXT("Add Empty Progress  %s"), *it.Key);
							FOnlineAchievementPico NewAchievement(it.Value);
							NowExistAchievements->Add(NewAchievement);
						}
					}
					Delegate.ExecuteIfBound(PicoPlayerId, true);
				}
...
void FOnlineAchievementsPico::QueryAchievementDescriptionsByNextUrl(const FUniqueNetId& PlayerId, FString InNextUrl, const FOnQueryAchievementsCompleteDelegate& Delegate)
{
	auto PicoPlayerId = static_cast<FUniqueNetIdPico>(PlayerId);
